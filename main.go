...
func main() {
	...
	var useWebsiteInformer, crtsh, orgcrt, whoisdomain, bigDomain bool
	...
	flag.BoolVar(&bigDomain, "b", false, "Fetch results from BigDomainData API")

	// Define usage function for the command line flags
	flag.Usage = func() {
		fmt.Println("\nAcqHunt - Domain Acquisition Discovery Tool (Created by Ractiurd)")
		fmt.Println("Usage: acqhunt [OPTIONS]")
		fmt.Println("\nPrimary Target Options (select one):")
		fmt.Println("  -d string    Search by domain (e.g., example.com)")
		fmt.Println("  -org string  Search by organization name (e.g., 'Google LLC')")
		fmt.Println("  -ip string   Search by IP range (CIDR format, e.g., 192.168.1.0/24)")
		fmt.Println("  -a string    Search by ASN number (e.g., AS15169)")

		fmt.Println("\nData Source Options:")
		fmt.Println("  -c           Search crt.sh for domain certificates")
		fmt.Println("  -oc          Search crt.sh for organization certificates")
		fmt.Println("  -i           Search website.informer.com for related domains")
		fmt.Println("  -w           Perform reverse WHOIS lookup")
		fmt.Println("  -b           Query BigDomainData API")

		fmt.Println("\nOutput Options:")
		fmt.Println("  -o string    Output file path (saves results to file)")

		fmt.Println("\nExamples:")
		fmt.Println("  acqhunt -d example.com -c -w -b   # Full domain analysis")
		fmt.Println("  acqhunt -org 'Google LLC' -oc     # Organization certificate search")
		fmt.Println("  acqhunt -ip 192.168.1.0/24        # Find domains in IP range")
		fmt.Println("  acqhunt -a AS15169                # Find domains by ASN")
	}
	...
	if *orgFlag != "" {
		...
		if !useWebsiteInformer && !crtsh && !whoisdomain && !bigDomain {
		...
		// Moved outside the websiteinformer/whois check
		if useWebsiteInformer || whoisdomain || bigDomain {
			...
			if bigDomain {
				if email == "no_email_found" {
					writeOutput(outputFile, "No email found to query BigDomainData\n")
				} else {

					domains := bigdomaindata(email)
					for _, domain := range domains {
						writeOutput(outputFile, fmt.Sprintf("%s\n", domain))
					}
				}
...
func bigdomaindata(email string) []string {
	// Helper to get user home directory
	...
	// Build API key filepath dynamically
	apiKeyFile := filepath.Join(getUserHomeDir(), "go", "pkg", "acq", "bigdomainapi.txt")
	...
	if err == nil && len(strings.TrimSpace(string(data))) > 0 {
		...
		fmt.Print("Enter your BigDomainData API key: ")
	...
	for {
		url := fmt.Sprintf("https://api.bigdomaindata.com/?key=%s&database=historical&registrant_email=%s", apiKey, email)
		if searchAfter != "" {
			url += fmt.Sprintf("&search_after=%s", searchAfter)
		}

		resp, err := http.Get(url)
		if err != nil {
			log.Fatalf("HTTP request failed on page %d: %v", page, err)
		}
		defer resp.Body.Close()

		body, err := io.ReadAll(resp.Body)
		if err != nil {
			log.Fatalf("Failed to read response on page %d: %v", page, err)
		}

		var parsed ApiResponse
		if err := json.Unmarshal(body, &parsed); err != nil {
			log.Fatalf("Failed to parse JSON on page %d: %v", page, err)
		}

		for _, domain := range parsed.Results {
			domains = append(domains, domain.DomainName)
		}

		if parsed.SearchAfter == "" {
			break
		}
